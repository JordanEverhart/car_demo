...
class NovaFrankaNavEnvCfg(DirectRLEnvCfg):
    ...
    # scene
    scene: InteractiveSceneCfg = InteractiveSceneCfg(num_envs=512, env_spacing=0, replicate_physics=True)
...
class NovaFrankaNavEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        self.actions = torch.clamp(self.action_scale * actions.clone(), min=-100.0, max=100.0)
    ...
    def _get_observations(self) -> dict:
        ...
        cur_angle = quaternion_to_angle(chassis_state[:,3:7])
        tar_angle = quaternion_to_angle(self.target_quat)
        obs = torch.cat(
            (
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        done = ((dist < self.cfg.dist_limit) & (angle < self.cfg.angle_limit)) | (dist > self.cfg.dist_max)
        # if (dist > self.cfg.dist_max).any():
        #     print("out range")
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # chassis_state = self.get_state_of_prim('chassis_link')
        # dist = torch.norm(self.target_pos - chassis_state[:, :2], dim=-1) 
        # print((dist.max()))
        # if (dist>40).any():
        #     print('sout_put_0')
...
def compute_rewards(
    ...
    dist_error = torch.norm(tar_pos - cur_pos, dim=-1)
        # 基础距离奖励：距离越小奖励越高
    reward_dist = (30-dist_error)**2
    reward_dist = torch.where(dist_error < 0.1, reward_dist + 100000.0, reward_dist)
    # reward_dist = torch.where(dist_error < 1.0, reward_dist + 5000000.0, reward_dist)
    # reward_dist = torch.where(dist_error < 3.0, reward_dist + 10000.0, reward_dist)
    # reward_dist = torch.where(dist_error < 5.0, reward_dist + 500.0, reward_dist)
    reward_dist = torch.where(dist_error > dist_max, reward_dist - 10000.0, reward_dist)
    ...
    # heading_error: 期望朝向与当前朝向之间的差值，归一化到 [-pi, pi]
    heading_error = (desired_yaw - current_yaw + math.pi) % (2 * math.pi) - math.pi
    reward_heading =  (torch.abs(torch.abs(heading_error) - math.pi/2))*(dist_error)
    ...
    # 四元数相似性对应于角度差：theta = 2 * acos(|dot|)
    yaw_diff = 2 * torch.acos(torch.abs(dot))
    reward_orient = (torch.pi - yaw_diff)
    reward_orient = torch.where(yaw_diff < 0.01, reward_orient + 10000000000.0, reward_orient)
    reward_orient = torch.where(yaw_diff < 0.1, reward_orient + 100000.0, reward_orient)
    # reward_orient = torch.where(yaw_diff < 0.5, reward_orient + 500000.0, reward_orient)
    # reward_orient = torch.where(yaw_diff < 1.0, reward_orient + 50000.0, reward_orient)
    # if (yaw_diff < 0.5 and dist_error < 0.1).any():
    #     print('HAVE05')
    ...
    # 增加时间惩罚（每个 step 都减一定值）
    time_penalty = -0.01
    total_reward = reward_dist+time_penalty+orientation_component
